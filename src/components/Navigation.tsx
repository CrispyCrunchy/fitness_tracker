...
export default function Navigation () {
  ...
  return (
    <div className="flex min-w-screen w-full absolute bottom-0 text-center">
      ...
      <Link href={user.isSuccess ? "/app/history/" + user.data.id : "/app"} className="bg-orange-500 hover:bg-orange-600 p-2 text-white border-black border-x basis-1/4 text-sm">History</Link>
      ...
      <button onClick={() => signOut()} className="bg-orange-500 hover:bg-orange-600 p-2 text-white border-black border-x basis-1/4 text-sm">Sign Out</button>
    </div>
  );
}